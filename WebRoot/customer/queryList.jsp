...
<script type="text/javascript">
	function number() {
		...
		for (var i = 0; i < ${pages}; i++) {
			res += "<a href='<%=basePath %>QueryListServlet?grade=${grade}&page="+ (i + 1) + "' class='active'>" + (i + 1) + " </a>";
		}
...
<div class="container">
    ...
    <div class="clearfloat"></div>
     ...
     <div class="today">
         ...
         <ul>
              <c:forEach var ="bookinfo"  items="${bookinfos}">
              	<li>
                        <a href="<%=basePath %>BookDetailServlet?bookid=${bookinfo.bookid}">
                             <img src="${bookinfo.bookimg}" width="178" height="168" alt="" title=""/>
     ...
     <div class="pages"> 
        <span class="disabled">&lt;&lt;
        <c:if test="${page>1}">
        <a href="queryListServlet?page=${page-1 }&grade=${grade}">上一页 &gt;&gt; </a>
        </c:if>
...
<!--         <span class="current">1</span> -->
        ...
        <c:if test="${page>=pages}">下一页</c:if>
       共${pages}页，到第
       <form action = "QueryListServlet"  method = "post">
       <input type="text" name="pageindex" class="ui-page-skipTo" size="3" value="1">页
        <button type="submit" class="ui-btn-s">确定</button></form>
     </div> 
    <div style="clear:both;"></div>
    <!--page begin -->
</div>
