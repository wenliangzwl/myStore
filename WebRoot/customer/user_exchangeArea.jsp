...
<script type="text/javascript">
	...
	function number() {
		...
		for (var i = 0; i < ${totalpage}; i++) {
			res += "<a href='<%=basePath %>User_exchangeAreaServlet?pageIndex="	+ (i + 1) + "' class='active'>" + (i + 1) + " </a>";
		}
...
$(document).ready(function(){
		...
		$(this).toggleClass("jia"); return false;
         ...
         <div class="user_cont_right">
           <div class="user_lmt">
                    <div class="user_lmt1">我的商品兑换中心</div>
           ...
           <div class="user_coment">
                 ...
                 <div class="my_dhzq">
                       ...
                       <div class="my_dhzq_b">
                            ...
                            <dl>
                                 <dt><img src="${list.exchangeimg}" width="104" height="141" /></dt>
                                 <dd>${list.exchangename}</dd>
                                 <dd>积分：<b class="red">${list.score}</b></dd>
                                 <dd><a href="<%=basePath %>User_exchangeArea_ManageServlet?exchangeid=${list.exchangeid}"><img src="<%=basePath %>customer/images/dh_but.jpg" width="68" height="24" /></a></dd>
                            </dl>
                          	</c:forEach>
                       </div>
                 </div>
           ...
           <!--分页开始 -->
                 <div style="clear:both; " ></div>
                   ...
                   <c:if test="${pageIndex>1}">
						<a href="<%=basePath %>User_exchangeAreaServlet?pageIndex=${pageIndex-1}">上一页</a>|		
		            </c:if>
					<c:if test="${pageIndex<=1}">上一页</c:if>
					<c:if test="${pageIndex<totalpage}">
						<a href="<%=basePath %>User_exchangeAreaServlet?pageIndex=${pageIndex+1}">下一页</a>
					</c:if>
