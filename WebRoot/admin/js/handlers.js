...
function init(uploadLimit,sizeLimit){
	...
	var uploadbody='<div id="uploadBody" class="ke-dialog-upload">'
		...
		+'		  <div class="ke-inline-block ke-swfupload-desc">允许用户同时上传&nbsp;<font color="red">'+uploadLimit+'</font>&nbsp;张图片，单张图片容量不超过<font color="red">'+sizeLimit+'</font>，建议图片最佳尺寸800*800 </div>'
...
//自定义图标删除方法
function delImg(objbook,objx){
	var add_url=contextPath+"/servlet/FileUploadServlet.do";
   	$.ajax({
		type:"POST",
		url:add_url,
		data:'fileid='+objbook.src,
...
function uploadSuccess(file, serverData) {
	try {
		var progress = new FileProgress(file);
		var s=jQuery("#userPhoto").attr("src");		
		var data = eval("(" + serverData + ")");
		$("#"+file.id).find(".ke-img").attr("data-status",data.status);
		if (data.status == 0 || data.status == "0") {
			
			//var imageli="<img  src='"+contextPath+data.message+"' height='80' width='100'/>";
			
			var imageli = "<div style='float:left;padding-left:5px'><img id='tobedel"+count+"' src='"+contextPath+data.message+"' height='80'	width='100' /></div><div style='float:left;margin-top:1px;margin-left:-12px'><img src='"+contextPath+"/images/delimg.bmp' style='cursor: pointer;' onclick='delImg(tobedel"+count+",this)'/></div>";
			count = count + 1;
			$(".photos li").append(imageli);
			...
			progress.setStatus("图片尺寸必须是800*800");
		}else{
			progress.setStatus("上传失败");
		}
...
function addReadyFileInfo(fileid,fileName,message){
	var photoDiv='<div class="ke-inline-block ke-item" id="'+fileid+'">'
			...
			+'	<img src="'+contextPath+'/images/image.png" class="ke-img" data-status="-1" width="100" height="100" />'
