...
public class Commentimpl implements Commentdao {
	...
	//添加评论之前判断是否已经下过订单
		...
		public boolean selectuserinfocondition(Userinfo userinfo,Bookinfo bookinfo){
			boolean bq=false;
			try {
				pstm=DBhelper.getConnection().prepareStatement(Document.COMMENTIMPLSELECTUSERINFOCONDITION);
...
select * from comment WHERE userid=1 ORDER BY commenttime desc LIMIT 0,5*/
	//dxp新添加  分页显示我的评论
		...
		public List<Comment> selectComment(Userinfo userinfo, int pag) {
			...
			pstm.setInt(1, userinfo.getUserid());
			System.out.println("底层测试userid"+userinfo.getUserid());
			pstm.setInt(2, (pag-1)*5);
			rs=pstm.executeQuery();
			list=new ArrayList<Comment>();
			while(rs.next()){
				...
				System.out.println("底层测试"+list.size());		
			return list;
		}
